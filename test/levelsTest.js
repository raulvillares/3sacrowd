...
function getAllSolutions (board) {
  ...
  const solveBoard = () => {
    ...
    if (moves.length === emptyCount) {  // Return if it's been solved
      solutions.push(JSON.parse(JSON.stringify(workBoard)));
      // console.log("FOUND SOLUTION");
    } else {  // Continue to guessing if not yet solved
      makePlay(workBoard, 
        moves, 
...
function checkPlay (board, row, col) {
  ...
  const patterns = [
    ...
    [[-1,size,1,size],[0,-1],[0,-2]], // two squares to the left
    [[1,size,-1,size],[-1,0],[-2,0]], // two squares up
    [[-1,size-2,-1,size],[1,0],[2,0]], // two squares down
    [[-1,size,-1,size-2],[0,1],[0,2]], // two squares to the right

    [[-1,size,0,size-1],[0,-1],[0,1]], // square left and right
    [[0,size-1,-1,size],[-1,0],[1,0]], // square above and below
    [[0,size-1,0,size-1],[-1,-1],[1,1]], // square to upper-left, lower-right
    [[0,size-1,0,size-1],[1,-1],[-1,1]], // square to lower-left, upper-right

    [[1,size,1,size],[-1,-1],[-2,-2]], // 2 squares up & left
    [[1,size,-1,size-2],[-1,1],[-2,2]], // 2 squares up & right
    [[-1,size-2,1,size],[1,-1],[2,-2]], // 2 squares down & left
    [[-1,size-2,-1,size-2],[1,1],[2,2]] // 2 squares down & right
  ];
  
  for (let p of patterns) {
    if( // checks position to see if the check can be performed (not too close to the edge)
      p[0][0] < row &&
      row < p[0][1] &&
      p[0][2] < col &&
      col < p[0][3]
    ){
      checkTwoSquares( // performs check for that specific pair
...
function surroundingCheck (board, rownum, colnum) {
  ...
  for (let r = startRow; r <= endRow; r++) {
    for (let c = startCol; c <= endCol; c++) {
      if (board[r][c] > 0 && board[r][c] < 3) { // only counting 1s and 2s
